...
def make_optimizer_and_scheduler(cfg, policy):
    if cfg.policy.name == "act":
        ...
        optimizer = torch.optim.AdamW(policy.parameters(), lr = cfg.training.lr, betas = [0.95, 0.999], weight_decay =cfg.training.weight_decay)
    ...
    elif cfg.policy.name == 'atm':
        optimizer = eval(cfg.training.optimizer_cfg.type)
        optimizer = optimizer(policy.parameters(), **cfg.training.optimizer_cfg.params)
        from torch.optim.lr_scheduler import CosineAnnealingLR

        lr_scheduler = eval(cfg.training.scheduler_cfg.type)
        lr_scheduler = lr_scheduler(optimizer,**cfg.training.scheduler_cfg.params)
    elif cfg.policy.name == 'snn':
        optimizer = torch.optim.AdamW(
...
def update_policy(
    ...
    # Forward automatically handled in mix precision when Fabric is applied
    output_dict = policy.forward(batch)
    loss = output_dict["loss"]
    # Loss is calculated under mix precision mode, have to scale
    fabric.backward(grad_scaler.scale(loss))
    ...
    # )
    grad_norm = 0.
    #Deepspeed handles grad clip by itself
...
def train(cfg: DictConfig, out_dir: str | None = None, job_name: str | None = None):
    ...
    assert cfg.training.offline_steps > 0 or cfg.training.training_epochs > 0

    if cfg.training.offline_steps == -1:
        cfg.training.offline_steps = (len(offline_dataset) // cfg.training.batch_size) * cfg.training.training_epochs
    ...
    def evaluate_and_checkpoint_if_needed(fabric, step, is_online):
        _num_digits = max(6, len(str(cfg.training.offline_steps + cfg.training.online_steps)))
        step_identifier = f"{step:0{_num_digits}d}"

        # No eval yet
        if cfg.training.eval_freq > 0 and step % cfg.training.eval_freq == 0 and fabric.is_global_zero:
    ...
    while True:
        ...
        for batch in tqdm(dataloader):
            ...
            losses += train_info["loss"]

            if step % cfg.training.log_freq == 0 and fabric.is_global_zero:
                log_train_info(logger, train_info, step, cfg, offline_dataset, is_online=False)

            # Note: evaluate_and_checkpoint_if_needed happens **after** the `step`th training update has completed,
            # so we pass in step + 1.
            
            evaluate_and_checkpoint_if_needed(fabric, step + 1, is_online=False)

            update_time_cum += train_info["update_s"]
            dataloading_time_cum += train_info["dataloading_s"]

            # print("update time:", update_time_cum)
            # print("dataloading time:", dataloading_time_cum)
    ...
    # Online and Fabric together not implemented yet
