    ...
    def teleop_step(
        ...
        for name in self.cameras:
            ...
            if type(img) == tuple:
                ...
                depths[name] = img[1]
                depths[name] = torch.from_numpy(depths[name])
            else:
                images[name] = img
                images[name] = torch.from_numpy(images[name])
                depths[name] = torch.zeros(images[name].shape[:2],dtype=torch.uint16)
        ...
        for name in self.cameras:
            ...
            obs_dict[f"observation.depths.{name}"] = depths[name]
    ...
    def capture_observation(self):
        ...
        for name in self.cameras:
            ...
            if type(img) == tuple:
                ...
                images[name] = torch.from_numpy(images[name])
                depths[name] = img[1]
                depths[name] = torch.from_numpy(depths[name]).float()
            else:
                images[name] = img
                images[name] = torch.from_numpy(images[name])
                depths[name] = torch.zeros(images[name].shape[:2],dtype=torch.uint16)
