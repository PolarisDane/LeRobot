...
from lerobot.common.datasets.track_utils import track_through_video
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        use_pointcloud: bool | None = False,
        ...
        self.camera_intrinsic = np.array(camera_intrinsic)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_pointcloud and (self.camera_intrinsic is not None): 
            ...
            images = torch.stack([item[k] for k in expected_image_keys], dim=-4) # v,c,h,w
            depths = torch.stack([item[k] for k in expected_depth_keys], dim=-4)
            depths = depths[:,0]*256*255+depths[:,1]*255
            voxel_size=0.005
            ...
            item['input_coords_list'] = input_coords_list
            item['input_feats_list'] = input_feats_list
    ...
    def load_point_cloud(self, rgb_image, depth_image, voxel_size):
        # 相机内参（根据你的相机进行设置）
    ...
    def from_preloaded(
        ...
        obj.use_pointcloud = use_pointcloud
        obj.camera_intrinsic = camera_intrinsic
        return obj
...
class MultiLeRobotDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.use_pointcloud:
            clouds=[]
            images = torch.stack([item[k] for k in expected_image_keys], dim=-4)
            depths = torch.stack([item[k] for k in expected_depth_keys], dim=-3)

            for i in range(images.shape[0]):
                points, colors = self.load_point_cloud(images[i], depths[i])
