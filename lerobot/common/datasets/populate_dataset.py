...
def save_image(img_tensor, key, frame_index, episode_index, videos_dir: str):
    ...
    if img_tensor.numpy().dtype == 'uint16':
        img_ = np.stack([img_tensor.numpy()/256,img_tensor.numpy()%256,np.zeros_like(img_tensor)],axis=2).astype(np.uint8)
...
def init_dataset(
    ...
    if local_dir.exists() and force_override:
        sub_del_dir_recursive(local_dir)
...
def add_frame(dataset, observation, action):
    ...
    img_keys = [key for key in observation if "image" in key or "depth" in key]
    non_img_keys = [key for key in observation if "image" not in key and "depth" not in key]
...
def sub_del_dir_recursive(dir_path):
...
def delete_current_episode(dataset):
    ...
    for tmp_imgs_dir in videos_dir.glob(f"*_episode_{episode_index:06d}"):
        sub_del_dir_recursive(tmp_imgs_dir)
...
def save_current_episode(dataset):
    ...
    for key in ep_dict:
        # print("key:", key)
        if "observation" in key and "image" not in key  and "depth" not in key:
            if ep_dict[key][0] is None:
                continue
...
def from_dataset_to_lerobot_dataset(dataset, play_sounds):
    ...
    if video:
        image_keys = [key for key in data_dict if "image" in key or "depth" in key]
