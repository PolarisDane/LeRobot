...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.use_images:
            ...
            self.backbones = []
            for _, key in enumerate(config.input_shapes.keys()):
                if "observation.images" in key:
                    backbone = IntermediateLayerGetter(backbone_model, return_layers={"layer4": "feature_map"}).cuda()
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.use_images:
            ...
            for cam_index in range(batch["observation.images"].shape[-4]):
                cam_features = self.backbones[cam_index](batch["observation.images"][:, cam_index])["feature_map"]
