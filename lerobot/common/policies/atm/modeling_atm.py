...
from lerobot.common.policies.atm.model import *
from lerobot.common.policies.atm.model.track_patch_embed import TrackPatchEmbed
from lerobot.common.policies.atm.vilt_modules.transformer_modules import *
from lerobot.common.policies.atm.vilt_modules.rgb_modules import *
from lerobot.common.policies.atm.vilt_modules.language_modules import *
from lerobot.common.policies.atm.vilt_modules.extra_state_modules import (
    ExtraModalityTokens,
)
from lerobot.common.policies.atm.vilt_modules.policy_head import *
from lerobot.common.policies.atm.configuration_atm import ATMConfig
from lerobot.common.policies.atm.utils.flow_utils import (
    ImageUnNormalize,
    sample_double_grid,
    tracks_to_video,
)
from lerobot.common.policies.atm.constants import TASK_EMB
...
class BCViLTPolicy(nn.Module,PyTorchModelHubMixin):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(
        ...
        if config is None:
            config = ATMConfig()
        self.config: ATMConfig = config
        print(config)

        self._process_obs_shapes(**config.model_cfg["obs_cfg"])

        # 1. encode image
        self._setup_image_encoder(**config.model_cfg["img_encoder_cfg"])

        # 2. encode language (spatial)
        self.language_encoder_spatial = self._setup_language_encoder(
            output_size=self.spatial_embed_size,
            **config.model_cfg["language_encoder_cfg"],
        )

        # 3. Track Transformer module
        self._setup_track(**config.model_cfg["track_cfg"])
        ...
        self._setup_spatial_transformer(**config.model_cfg["spatial_transformer_cfg"])

        ### 5. encode extra information (e.g. gripper, joint_state)
        self.extra_encoder = self._setup_extra_state_encoder(
            extra_embedding_size=self.temporal_embed_size,
            **config.model_cfg["extra_state_encoder_cfg"],
        )
        ...
        self.language_encoder_temporal = self._setup_language_encoder(
            output_size=self.temporal_embed_size,
            **config.model_cfg["language_encoder_cfg"],
        )

        # 7. define temporal transformer
        self._setup_temporal_transformer(**config.model_cfg["temporal_transformer_cfg"])

        # 8. define policy head
        self._setup_policy_head(**config.model_cfg["policy_head_cfg"])

        if config.model_cfg["load_path"] is not None:
            self.load(config.model_cfg["load_path"])
            self.track.load(
                f"{ config.model_cfg['load_path']['track_fn']}/model_best.ckpt"
            )
    ...
    def model_forward(self, obs, track_obs, track, task_emb, extra_states):
        """
    ...
    def forward(self, batch):
        ...
        if len(self.expected_image_keys) > 0:
            ...
            batch["observation.images"] = (
                torch.stack([batch[k] for k in self.expected_image_keys], dim=-4) * 255
            )
        batch_size = (
            batch["observation.images"]
        ...
        b, t, v, c, h, w = batch["observation.images"].shape
        batch["observation.images"] = (
            F.interpolate(
                batch["observation.images"].reshape(b * t * v, c, h, w),
                size=self.config.img_size,
                mode="bilinear",
                align_corners=False,
            )
            .reshape(b, t, v, c, *self.config.img_size)
            .transpose(1, 2)
        )
        obs = torch.stack(
            [
                batch["observation.images"][:, :, self.policy_num_track_ts + t - 1]
                for t in range(self.config.frame_stack)
            ],
            dim=2,
        )

        track_obs = torch.stack(
            [
                batch["observation.images"][:, :, t : self.policy_num_track_ts + t]
                for t in range(self.config.frame_stack)
            ],
            dim=2,
        )
        ...
        dist = self.model_forward(obs, track_obs, track, task_emb, extra_states)
    ...
    def forward_vis(self, obs, track_obs, track, task_emb, extra_states, action):
        ...
        for view in range(self.num_views):
            ...
            combined_gt_track_vid = (
                (track_obs[:1, view, 0, :, ...] * 0.25 + gt_track_vid * 0.75)
                .cpu()
                .numpy()
                .astype(np.uint8)
            )

            _, ret_dict = self.track.forward_vis(
                track_obs[:1, view, 0, :, ...],
                grid_track[:1, view],
                task_emb[:1],
                p_img=0,
    ...
    def select_action(self, batch):
        ...
        b, v, c, h, w = batch["observation.images"].shape

        # expand time dimenstion
        obs = rearrange(batch["observation.images"], "b v c h w -> b v 1 c h w").clone()
        # extra_states = {
        ...
        task_emb = torch.tensor(
            TASK_EMB,
        ).repeat(b, 1).to(device=device, dtype=dtype)
        # extra_states = {
        ...
        with torch.no_grad():
            ...
            action = action  # (b, act_dim)

        action = action.reshape(-1, *self.act_shape)
        action = torch.clamp(action, -1, 1)
        return action # (b, *act_shape)
